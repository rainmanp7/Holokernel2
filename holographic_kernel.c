...
#define MAX_ALLOCATIONS         512      // Pre-allocated metadata pool
...
static void print_hex_vga(uint32_t value, uint8_t color, int row, int start_col) {
    ...
    int col = start_col;
    print_char_vga('0', color, row, col++);
    print_char_vga('x', color, row, col++);
    for (int i = 0; i < 8; i++) {
        int shift = (7 - i) * 4;
...
// --- MEMORY ALLOCATION ---
// ============================================================================
static void initialize_metadata_pool(void) {
    for (int i = 0; i < MAX_ALLOCATIONS - 1; i++) {
        metadata_pool[i].next = &metadata_pool[i + 1];
    }
    metadata_pool[MAX_ALLOCATIONS - 1].next = NULL;
    free_metadata_list = &metadata_pool[0];
}
...
static float compute_similarity(HyperVector* a, HyperVector* b) {
    ...
    if (a->active_dims == 0 || b->active_dims == 0) return 0.0f;
...
void kmain(void) {
    ...
    const uint32_t REPORT_INTERVAL = 3000000;
